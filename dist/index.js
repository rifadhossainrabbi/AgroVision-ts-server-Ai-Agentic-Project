...
async function generateTextWithAI(prompt) {
    ...
    if (googleApiKey) {
        const genAI = new generative_ai_1.GoogleGenerativeAI(googleApiKey);
        const aiModel = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
        const result = await aiModel.generateContent(prompt);
        return result.response.text().trim();
    }
    throw new Error('No valid AI API key found. Please set GROK_API_KEY or GOOGLE_API_KEY in agro-vision-server-ts/.env');
}
...
async function analyzeImageWithAI(prompt, base64Image, mimeType) {
    const grokApiKey = process.env.GROK_API_KEY;
    if (grokApiKey) {
        const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            method: 'POST',
    ...
    throw new Error('No valid AI API key found for image analysis. Please set GROK_API_KEY (vision-capable) or GOOGLE_API_KEY / GEMINI_API_KEY in agro-vision-server-ts/.env');
}
// --- 1D. STRUCTURED JSON AI HELPER (text -> JSON) ---
// Powers the AI Farm Analyzer feature.
async function generateJSONWithAI(prompt) {
    const grokApiKey = process.env.GROK_API_KEY;
    if (grokApiKey) {
        const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            method: 'POST',
...
async function run() {
    try {
        ...
        async function deleteProductCascading(productId) {
            if (mongodb_1.ObjectId.isValid(productId)) {
                await productsCollection.deleteOne({ _id: new mongodb_1.ObjectId(productId) });
            }
        ...
        app.patch('/api/products/:id', verifyToken, async (req, res) => {
            try {
                ...
                const result = await productsCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { ...updatedData, updatedAt: new Date() } });
        ...
        app.get('/api/products/:id', async (req, res) => {
            try {
                ...
                let sellerUser = null;
                if (product.userId) {
                    sellerUser = await usersCollection.findOne(mongodb_1.ObjectId.isValid(product.userId)
                        ? { _id: new mongodb_1.ObjectId(product.userId) }
                        : { _id: product.userId });
                }
                const seller = {
                ...
                const productComments = await commentsCollection
                    ...
                    .sort({ createdAt: -1 })
                    .toArray();
                const reviews = productComments.map((c) => ({
                    id: c._id.toString(),
                    user: c.userName,
                    rating: c.rating || 5,
                    comment: c.comment,
        ...
        app.patch('/api/admin/products/:id/status', verifyToken, verifyAdmin, async (req, res) => {
            try {
                ...
                const result = await productsCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { status, updatedAt: new Date() } });
                res.send({
                    success: true,
                    ...
                    if (!target?.isFeatured) {
                        ...
                        if (featuredCount >= 6) {
                            return res.status(400).send({
                                success: false,
                                error: 'Maximum 6 products can be featured on the home page. Please unfeature one first.',
                            });
                        }
                    }
                }
                const result = await productsCollection.updateOne({ _id: new mongodb_1.ObjectId(id) }, {
                    $set: {
                        isFeatured: !!featured,
        ...
        app.delete('/api/admin/users/:userId', verifyToken, verifyAdmin, async (req, res) => {
            try {
                ...
                if (mongodb_1.ObjectId.isValid(userId)) {
                    await usersCollection.deleteOne({ _id: new mongodb_1.ObjectId(userId) });
                }
                ...
                for (const doc of allLikes) {
                    const updatedLikedBy = (doc.likedBy || []).filter((u) => !((typeof u === 'object' &&
                        (u.userId === userId || u.userName === userId)) ||
                        u === userId));
                    await likesCollection.updateOne({ _id: doc._id }, { $set: { likedBy: updatedLikedBy } });
                }
        ...
        app.post('/api/ai/generate-description', verifyToken, async (req, res) => {
            try {
                ...
                const details = productType === 'Machine'
...
- Sound natural and persuasive, suitable for buyers browsing a marketplace.`;
        ...
        app.post('/api/ai/chat', verifyToken, async (req, res) => {
            try {
                ...
                let toolContext = '';
                const looksLikeProductQuery = /price|cost|available|stock|find|search|buy|sell|crop|machine|tractor|rice|wheat|fruit|vegetable/i.test(message);
                if (looksLikeProductQuery) {
                    ...
                    if (matches.length) {
                        toolContext = `\n\nLive marketplace data (use these real facts; never invent prices or availability):\n${matches
                            .map((p) => `- ${p.title} | ${p.category} | Price: ${p.price}/${p.unit} | Location: ${p.location || 'N/A'}`)
                            .join('\n')}`;
                    }
                }
...
- Keep each array to 2-5 short, practical items.`;
                const raw = await analyzeImageWithAI(prompt, cleanBase64, mimeType || 'image/jpeg');
                ...
                if (userId) {
                    ...
                    if (pastProducts.length) {
                        farmerContext = `Farmer's recent listings on AgroVision: ${pastProducts
                            .map((p) => `${p.title} (${p.category}, ${p.price}/${p.unit})`)
                            .join('; ')}.`;
                    }
                }
...
- Numbers should be realistic estimates, not placeholders like 0 or 100 unless truly appropriate.`;
        ...
        app.post('/api/likes/toggle', verifyToken, async (req, res) => {
            try {
                ...
                const likedByList = existing.likedBy || [];
                const alreadyLiked = likedByList.some((u) => (typeof u === 'object' &&
                    (u.userId === userId || u.userName === userName)) ||
                    u === userName);
                if (alreadyLiked) {
                    const updatedList = likedByList.filter((u) => !((typeof u === 'object' &&
                        (u.userId === userId || u.userName === userName)) ||
                        u === userName));
                    await likesCollection.updateOne({ productId }, { $set: { likedBy: updatedList } });
                    res.send({
                        success: true,
                        liked: false,
                    ...
                    await likesCollection.updateOne({ productId }, { $push: { likedBy: userObj } });
                    res.send({
                        success: true,
                        liked: true,
        ...
        app.get('/api/likes/:productId', async (req, res) => {
            ...
            const isLiked = likedBy.some((u) => (typeof u === 'object' &&
                (u.userId === userId || u.userName === userName)) ||
                u === userName);
            res.send({
        ...
        app.delete('/api/comments/:id', verifyToken, async (req, res) => {
            ...
            if (!mongodb_1.ObjectId.isValid(id))
                return res.status(400).send({ error: 'Invalid ID' });
            await commentsCollection.deleteOne({ _id: new mongodb_1.ObjectId(id) });
        ...
        app.post('/api/buy-requests', verifyToken, async (req, res) => {
            try {
                ...
                if (existingDoc) {
                    ...
                    await buyRequestsCollection.updateOne({ productId }, {
                        $push: {
                            users: {
                                userId: user.userId,
        ...
        app.get('/api/buy-requests/user/:userId', verifyToken, async (req, res) => {
            try {
                ...
                const requests = docs.map((doc) => {
                    const userReq = doc.users?.find((u) => u.userId === userId);
                    return {
        ...
        app.patch('/api/buy-requests/status', verifyToken, async (req, res) => {
            try {
                ...
                const result = await buyRequestsCollection.updateOne({ productId, 'users.userId': userId }, { $set: { 'users.$.status': status } });
                res.send({
                    success: true,
        ...
        app.delete('/api/buy-requests/:productId/:userId', verifyToken, async (req, res) => {
            try {
                ...
                const userId = req.params.userId;
                await buyRequestsCollection.updateOne({ productId }, { $pull: { users: { userId } } });
        ...
        app.post('/api/cart', verifyToken, async (req, res) => {
            try {
                ...
                if (existingDoc) {
                    ...
                    await cartCollection.updateOne({ productId }, {
                        $push: {
                            users: {
                                userId: user.userId,
                                userName: user.userName || 'Anonymous',
        ...
        app.get('/api/cart/:userId', verifyToken, async (req, res) => {
            try {
                ...
                const cartItems = docs.map((doc) => {
                    const userCart = doc.users?.find((u) => u.userId === userId);
                    return {
                        _id: doc._id.toString(),
                        productId: doc.productId,
                        productTitle: doc.productTitle,
        ...
        app.delete('/api/cart/:productId/:userId', verifyToken, async (req, res) => {
            try {
                const productId = req.params.productId;
                const userId = req.params.userId;
                await cartCollection.updateOne({ productId }, { $pull: { users: { userId } } });
...
app.get('/', (_req, res) => res.send('🌾 AgroVision API is Live'));
